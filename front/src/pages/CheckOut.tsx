...
export default function CheckOutPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const turmaExisteNoServidor = turmas.some(
      (item) => !item.isTemp && isSameTurmaName(item.nome, nomeTurma)
    );
    ...
    if (
      ...
      newErrors.turma = "Salve a edição da turma antes de liberar o check-out.";
    }

    if (turmaSelecionada?.isTemp || (!turmaExisteNoServidor && nomeTurma)) {
      newErrors.turma =
        "Turmas novas precisam passar pelo primeiro check-in antes de liberar check-out.";
    }
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:8080/api/avaliacao/liberar-checkout",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      resetTurmaEditor();
      setErrors({});
      toast.success("Código de check-out liberado!", { duration: 3000 });
    } catch (error) {
      console.error("Erro ao gerar código de check-out:", error);
      setErrors((prev) => ({
        ...prev,
        geral: "Falha ao liberar check-out. Tente novamente.",
      }));
      toast.error("Falha ao liberar check-out.", { duration: 3000 });
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const handleAdicionarTurma = () => {
    ...
    clearError("turma");
    toast.success(
      "Turma adicionada localmente. Faça o primeiro check-in dela antes do check-out.",
      {
        duration: 3500,
      }
    );
    ...
    <div className="flex flex-col min-h-screen w-full bg-gray-50">
      ...
      <main className="flex flex-1 w-full flex-col items-center px-4 py-6 md:px-8 md:py-10 pb-24 md:pb-10">
        <div className="w-full max-w-md">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-2">
                {selectedTurmaId !== null
                  ...
                  : "Turmas novas precisam passar pelo primeiro check-in antes do primeiro check-out."}
              </p>
            </div>
                    ...
                    >
                      ...
                      {item.isTemp && (
                        <span className="text-[11px] text-amber-600">
                          Local. Gere um check-in antes do check-out
                        </span>
                      )}
                    </button>
          ...
          {codigoGerado && <SuccessCard codigo={codigoGerado} tipo="checkout" />}
        </div>
      </main>
